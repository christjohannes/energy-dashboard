...
export const DonutChart = ({ width, height, data, hoveredGroup, setHoveredGroup, hoveredYear }) => {

  const radius = Math.min(width - 2 * MARGIN_X, height - 2 * MARGIN_Y) / 2;
  const innerRadius = radius / 2;
  const year = hoveredYear ?? 2024;
  ...
  const shapes = pie.map((group, i) => {
    ...
    const showLabel = (group.endAngle - group.startAngle) > 0.1 || hoveredGroup === group.data.name;
  ...
  const hoveredData = filtered.find(d => d.name === hoveredGroup);
